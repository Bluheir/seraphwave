...
export class MainImpl implements CreateCode, GetMetaInfo, FullApi, VoiceClientMgr {
    ...
    async createClient(info: MetaInfo & any, initialState: InitialConnState): Promise<VoiceClient> {
        const ws = new WebSocket(info.webSocketUrl)
        return new MainVoiceClient(ws, initialState)
    }
}
...
export class MainVoiceClient implements VoiceClient {
	...
	// events
	private onUserDataI: ((data: SessionCodeReceive) => Promise<void>) | ((data: SessionCodeReceive) => void)
    ...
    onSessionCode(ev: ((data: SessionCodeReceive) => Promise<void>) | ((data: SessionCodeReceive) => void)) {
		this.onUserDataI = ev
	}
